...
const checklistItemMdComponents: Components = {
  ...
  a: ({ href, children }) => (
    <a
      ...
      className="text-plan hover:underline underline-offset-2"
    >
      {children}
    </a>
  ),
  ...
  code: ({ children }) => (
    <code className="text-plan bg-elevated/80 px-1.5 py-0.5 rounded text-[12px] font-mono">
      {children}
    </code>
  ),
};
 ...
 */
function parseCategoryInfo(category: string): {
  name: string;
  timeBudget: string | null;
} {
  const timeMatch = category.match(
    /^(.+?)\s*\(([^)]+(?:hrs?|hours?|mins?|minutes?)[^)]*)\)\s*$/i,
  );
  if (timeMatch) {
...
function shouldShowPlanStatusBanner(
...
function PlanRoadmapBadges({
  ...
  const baseBadgeClass =
    "h-5 cursor-default pointer-events-none px-2 text-[10px] font-medium";
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      {planStatus === "ready" && (
        <Badge
          variant="outline"
          className={`${baseBadgeClass} border-success/40 bg-success/10 text-success`}
        >
          Plan up to date
        </Badge>
      )}
      {planStatus === "not_queued" && (
        <Badge
          variant="outline"
          className={`${baseBadgeClass} border-info/40 bg-info/10 text-info`}
        >
      ...
      {(planStatus === "queued" || planStatus === "running") && (
        <Badge
          variant="outline"
          className={`${baseBadgeClass} border-warning/40 bg-warning/10 text-warning animate-pulse`}
        >
      ...
      {planStatus === "failed" && (
        <Badge
          variant="outline"
          className={`${baseBadgeClass} border-danger/40 bg-danger/10 text-danger`}
        >
          Status: Failed
        </Badge>
      )}
  ...
  return (
    <TooltipProvider delay={180}>
      <Tooltip>
        ...
        >
          <RefreshCw
            className={`h-3.5 w-3.5 ${isQueueingPlanRefresh ? "animate-spin" : ""}`}
          />
          {buttonLabel}
        </TooltipTrigger>
        <TooltipContent
          side="top"
          className="max-w-[18rem] bg-surface border-border/80 shadow-xl"
        >
          Regenerates your game plan using your latest profile updates. Progress
          remains intact.
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 mb-3">
      {metadata.map((item, i) => {
        ...
        return (
          <div
            key={i}
            className={`flex flex-col p-2.5 rounded-xl border border-border/40 bg-surface/50 backdrop-blur-sm ${isProfile ? "col-span-full shadow-sm" : ""}`}
          >
            <span className="text-[9px] uppercase tracking-wider font-bold text-muted-foreground/60 mb-1.5 ml-0.5">
              {item.key}
            </span>
            {hasTags ? (
...
function PlanDashboardTable({
  ...
  dashboard: ParsedPlan["dashboard"];
}) {
          ...
          <thead>
            <tr className="bg-elevated/10">
              {dashboard.headers.map((h) => (
                <th
                  key={h}
                  className="px-3 py-1.5 border-r border-border/10 last:border-0 font-semibold text-muted-foreground"
                >
                  {h}
                </th>
              ))}
            ...
            {dashboard.rows.map((row, i) => (
              <tr key={i} className="hover:bg-elevated/5 transition-colors">
                {dashboard.headers.map((h) => (
                  <td
                    key={h}
                    className="px-3 py-1.5 border-r border-border/10 last:border-0 font-medium"
                  >
                    {row[h].includes("🔴") ||
                    row[h].includes("🟡") ||
                    row[h].includes("🟢") ? (
                      <Badge
                        variant="outline"
                        className="text-[9px] py-0 h-4 px-1 border-border/40 bg-surface"
                      >
                        {row[h]}
                      </Badge>
                    ) : (
...
function PlanCheckItem({
  ...
  const normalizedItemText = useMemo(
    () => normalizeMarkdownContent(item.text),
    [item.text],
  );
  ...
  return (
    <div className="flex items-start gap-2 py-1 transition-all duration-200">
      ...
      <div
        className={`text-[12px] leading-snug ${item.checked ? "line-through text-muted-foreground/60" : "text-foreground/90"} ${isInfoItem ? "font-bold mt-1.5 text-foreground mb-0.5 tracking-tight" : ""}`}
      >
        <ReactMarkdown
          remarkPlugins={[remarkGfm]}
          components={checklistItemMdComponents}
        >
          {normalizedItemText}
        </ReactMarkdown>
      </div>
...
function MonthProgress({ items }: { items: PlanItem[] }) {
  ...
  const done = items.filter(
    (i) => !i.id.includes("-info-") && i.checked,
  ).length;
...
export default function PlanPage() {
  ...
  const { data: rawData, mutate: mutatePlan } = useSWR(
    "/api/memory?file=plan.md",
    fetcher,
    {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- SWR callback type
      refreshInterval: (data: any) => {
        ...
        return status === "queued" ||
          status === "running" ||
          (content.includes(PLAN_PLACEHOLDER_MARKER) && status !== "failed")
          ? 10000
          : 0;
      },
    },
  ...
  const plan = useMemo(
    () => (rawData ? parsePlan(rawContent) : null),
    [rawContent, rawData],
  );

  const planStatus: OnboardingPlanStatus | null =
    statusData?.plan?.status ?? null;
  const planErrorCode: string | null =
    manualErrorCode ?? statusData?.plan?.lastErrorCode ?? null;
  ...
  const handleToggle = useCallback(
    (itemId: string, checked: boolean) => {
      // 1. Optimistic: derive update from latest cache to handle rapid clicks
      mutatePlan(
        (current: { content?: string } | undefined) => {
          const content =
            typeof current?.content === "string" ? current.content : rawContent;
          return { content: togglePlanItem(content, itemId, checked) };
        },
        { revalidate: false },
      );

      // 2. Fire API in background, revalidate on any outcome for reconciliation
  ...
  const canRequestRefresh =
    planStatus !== null && planStatus !== "queued" && planStatus !== "running";
  const isStructured = plan.months.length > 0 || plan.immediateSteps.length > 0;
  const normalizedFallbackContent = normalizeMarkdownContent(
    rawContent.replace(/^# .+\n{1,2}/, "").trim(),
  );
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2 border-b border-border/20 pb-4 mb-4">
        <div className="space-y-0.5">
          <div className="flex items-center gap-1.5 mb-0.5">
            <Badge
              variant="outline"
              className="bg-plan/10 text-plan border-plan/20 text-[10px] font-medium h-5 px-2 uppercase tracking-wider"
            >
              Roadmap
            </Badge>
            <PlanRoadmapBadges
        ...
        <div className="space-y-6">
                  ...
                  >
                    <div className="relative overflow-hidden rounded-xl border border-border/30 bg-gradient-to-br from-surface to-elevated/40 p-4 sm:p-5 shadow-lg">
                      ...
                      <div className="relative z-10">
                        <span className="text-[9px] font-bold uppercase tracking-widest text-plan block mb-1">
                          Focus
                        </span>
                        <h2 className="text-lg font-bold text-foreground mb-2 sm:text-xl tracking-tight">
                          {month.title}
                        </h2>
                        {month.theme && (
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {Object.entries(week.categories).map(
                                  ([category, items]) => {
                                    const { name: categoryName, timeBudget } =
                                      parseCategoryInfo(category);
                                    const domain =
                                      CATEGORY_DOMAINS[categoryName] ||
                                      CATEGORY_DOMAINS[
                                        Object.keys(CATEGORY_DOMAINS).find(
                                    ...
                                    return (
                                      <Card
                                        key={category}
                                        className={`overflow-hidden border border-border/20 bg-surface/60 transition-all hover:bg-surface/80 hover:shadow-lg ${categoryName.includes("Goals") ? "md:col-span-2" : ""}`}
                                      >
                                        <CardHeader className="py-2.5 px-4 border-b border-border/10 bg-elevated/20">
                                          <CardTitle className="text-[12px] font-bold flex items-center gap-2">
                                            <div
                                              className={`p-1 rounded-md ${colors.bg} ${colors.text} shadow-sm border ${colors.border}`}
                                            >
                                            ...
                                            {timeBudget && (
                                              <Badge
                                                variant="outline"
                                                className="shrink-0 text-[9px] h-5 border-plan/30 bg-plan/10 text-plan font-medium"
                                              >
                                                {timeBudget}
                                              </Badge>
                                            )}
                                            <Badge
                                              variant="outline"
                                              className="shrink-0 ml-auto text-[9px] h-5 border-border/40 bg-surface/50 font-bold tracking-tighter"
                                            >
                                              {
                                                items.filter(
                                                  (i) =>
                        ...
                        {Object.entries(month.categories).map(
                          ([category, items]) => {
                            ...
                            const domain =
                              CATEGORY_DOMAINS[categoryName] ||
                              CATEGORY_DOMAINS[
                                Object.keys(CATEGORY_DOMAINS).find((k) =>
                ...
                <CardContent className="p-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {plan.immediateSteps.map((item) => (
                      <div
                        key={item.id}
                        className="p-2 rounded-lg bg-surface/50 border border-streak/10 hover:border-streak/30 transition-all"
                      >
              ...
              <CardContent className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {plan.redFlags.map((flag, i) => (
                    <div
                      key={i}
                      className="flex gap-2 p-3 rounded-lg border border-border/20 bg-surface hover:shadow-md transition-all border-l-2 border-l-danger"
                    >
                      <div className="space-y-1">
                        ...
                        <div className="flex items-start gap-1.5 text-[11px] text-muted-foreground/80 leading-snug bg-elevated/40 p-2 rounded-md border border-border/10">
                          ...
                          <span>
                            <span className="font-bold text-streak mr-1">
                              Fix:
                            </span>{" "}
                            {flag.fix}
                          </span>
          ...
          <div className="mt-6 flex items-start gap-2 p-3 rounded-lg border border-border/15 bg-elevated/20">
            ...
            <p className="text-[11px] leading-relaxed text-muted-foreground/60">
              This game plan is AI-generated based on your profile and
              preferences. Some information may be outdated or not entirely
              relevant to your current situation. Links and recommendations are
              provided as guidance — please verify details independently before
              acting on them.
            </p>
          </div>
        </div>
